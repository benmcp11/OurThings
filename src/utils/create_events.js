...
const is_authenticated = () => {
    return document.getElementById("is_authenticated_div") !== null
}
...
const update_warnings = () => {
    ...
    ANONYMOUS = document.getElementById("username").innerHTML === "Anonymous"
    ...
    if (ANONYMOUS) {
        warnings += "WARNING: Consider modifying the author.<br>"
    }
...
const url_lookup = () => {
    ...
    const submit_qu = document.getElementById("save-submit-qu")
    if (!url_val){
        disp.innerHTML = "Url must not be blank"
        disp.style.color = 'red'
        submit.hidden = true
        submit_qu.hidden = true
        return 
    }
    fetch("url_check", {
        method: "POST",
...
const update_author_name = () => {
    const email = document.getElementById("id_email").value
    document.getElementById("author_email").value = email
    if (email === "") {
        document.getElementById("username").innerHTML = "Anonymous"
        document.getElementById("anonymousAuthorModalIcon").hidden = false
        document.getElementById("authorInfoModalIcon").hidden = true
    }
    else {
        document.getElementById("username").innerHTML = email.split("@")[0]
        document.getElementById("anonymousAuthorModalIcon").hidden = true
        document.getElementById("authorInfoModalIcon").hidden = false
    }
    set_url_dirty()
    update_warnings()
    resetSignupErrors()
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById("save-submit-qu").addEventListener("click", () => {
        submitTwoForms("quick-signup", "save")
    })

    if (document.getElementById("id_email")){
        // Not logged in
        document.getElementById("id_email").addEventListener("input", () => {
            console.log("email type event firing")
            update_author_name()
        })
    }
    else {
